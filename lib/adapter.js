...
// Dependencies
var pg = require('pg'),
    pgTransaction = require('pg-transaction'),
...
module.exports = (function() {
    ...
    var adapter = {
        ...
        define: function(table, definition, cb) {
            ...
            spawnConnection(function __DEFINE__(client, cb) {
                ...
                var indexes = utils.buildIndexesType(definition);
                ...
                client.query(query, function __DEFINE__(err, result) {
                    ...
                    function buildIndex(index, cb) {
                        // USING statment for non-standard index engine
                        var using = index.type !== 'btree' ? ' USING ' + index.type : '';
                        ...
                        var query = 'CREATE INDEX ' + cleanTable + '_' + index.name + ' on ' + table + using + ' (' + index.name + ');';
        ...
        transaction: function(config, cb) {
            var self = this;
            ...
            pg.connect(dbConfig, function(err, client, done) {
                ...
                tr.trClient = new pgTransaction(client);
                tr.client = tr.trClient.client;
    ...
    _.extend(Transaction.prototype, adapter, {
        // Raw Query Interface
        query: function(table, query, data, cb) {
        ...
        // Rename table
        rename: function(currentTable, newTable, cb) {
            var client = this.client;

            // Escape Table Names
            currentTable = utils.escapeTable(currentTable);
            newTable = utils.escapeTable(newTable);

            // Build Query
            var query = 'ALTER TABLE ' + currentTable + ' RENAME TO ' + newTable;

            // Run Query
            client.query(query, function __RENAME__(err, result) {
                if (err) result = null;
                cb(null, result);
            });
        },
        ...
        // batch import from stream
        createBatch: function(options, columns, cb) {
            var client = this.client,
                ...
                rowBufferCount = 0,
                queryParts = [],
                rowBufferSize = options.rowBufferSize || 1000,
                table = _.isString(options) ? options : options.table;
            ...
            var query = 'INSERT INTO ' + table + ' (' + columnNames.join(', ') + ') VALUES';

            var buildValues = function(row, next) {
                var values = [];
                columns.forEach(function(col, idx) {
                    ...
                    if (value === null) {
                        ...
                        //TODO better protection against SQL injection
                        values.push("'" + utils.prepareValue(value).replace(/'/g, "''") + "'");
                    }
                });
                queryParts.push('(' + values.join(', ') + ')');
                next();
            };

            var sendQuery = function(next) {
                var _query = query + "\n" + queryParts.join(",\n") + ';';
                client.query(_query, function(err) {
                    if (err) {
                        console.log(_query);
                        return next(err);
                    }
            ...
            inputStream.on('row', function(row, next) {
                buildValues(row, function() {
                    if (queryParts.length >= rowBufferSize) {
                        sendQuery(next);
                    } else {
                        next();
                    }
            ...
            inputStream.on('end', function(next) {
