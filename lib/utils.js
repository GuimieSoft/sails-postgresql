...
utils.buildIndexesType = function(obj) {
  var indexes = [];

  // Support B-tree and non-standard index engines
  var types = ['btree', 'hash', 'gin', 'gist'];
  ...
  Object.keys(obj).forEach(function(key) {
    if(obj[key].hasOwnProperty('index')) {
      indexes.push({
        name: key,
        type: ~types.indexOf(obj[key].index) ? obj[key].index : types[0]
      });
    }
  });
...
utils.sqlTypeCast = function(type) {
  // Allow Geometry and Geography types
  if( /^(geometry|geography)/i.test(type) ) {
    return type;
  }
